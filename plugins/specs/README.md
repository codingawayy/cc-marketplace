...
Run this first to create the overview, then use `/specs:index-artifacts` to index artifacts.

### `/specs:index-artifacts`

Analyzes your codebase and generates `.specs/artifacts.csv` - an index of system artifacts.
...
Generates YAML specification files to `docs.specs/` using the artifact index as a guide.

```
/specs:generate-specs
```

Requires `.specs/overview.md` and `.specs/artifacts.csv` to exist. Run `/specs:generate-overview` and `/specs:index-artifacts` first.
...
The plugin uses the `.specs/` directory to store configuration and intermediate files:

```
.specs/
├── config.json      # Exclusion patterns and commit tracking
├── overview.md      # System architecture overview (generated by /specs:generate-overview)
├── artifacts.csv    # Index of artifacts: system, entity, action, task, service, app
└── temp/            # Generation plans and change logs
```
...
The plugin uses `.specs/config.json` to configure exclusion patterns. This file is created automatically when you first run `/specs:generate-overview`.
